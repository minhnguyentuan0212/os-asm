...
static void * cpu_routine(void * args) {
	...
	while (1) {
		...
		if (proc == NULL && done) {
			...
			if (proc->priority != MAX_PRIO - 1) printf("\tCPU %d: Dispatched process %2d from queue with prio %d\n",
				id, proc->pid, proc->priority - 1);
			else printf("\tCPU %d: Dispatched process %2d from queue with prio %d\n",
				id, proc->pid, proc->priority);
			if (proc->priority == MAX_PRIO - 1) time_left = time_slot;
			else time_left = time_slot * proc->priority;
		}
